const CryptonToken = artifacts.require('CryptonToken');
...
contract('CryptonToken: test mint and lock', (accounts) => {
    ...
    it('is possible to mint tokens for the minter role', async () => {
        let token = await CryptonToken.deployed();
    ...
    it('is not possible to unlock tokens for anybody else than the token holder', async () => {
        ...
        await truffleAssert.fails(
            ...
            truffleAssert.ErrorType.REVERT,
            'CryptonToken: Only the Owner can unlock the Token',
        );
    });
    ...
    it('is not possible to unlock tokens without the correct unlock code', async () => {
        ...
        await truffleAssert.fails(
            ...
            truffleAssert.ErrorType.REVERT,
            'CryptonToken: Unlock Code Incorrect',
        );
    });
